...
export default function ChatWidget() {
  ...
  const barBorder = isDark ? "border-slate-500" : "border-gray-400";
    ...
    return (
      <div style={widgetStyle}>
        <button
          ...
          style={isDark ? { backgroundColor: "#1e293b" } : undefined}
        >
  ...
  // Expanded: same position, card (friends list or chat)
  return (
    <div style={widgetStyle}>
      {!isAuthenticated ? (
        <div className={`w-[360px] rounded-lg border-2 p-6 ${barBg} ${barBorder} ${barText}`} style={{ boxShadow: "0 10px 40px rgba(0,0,0,0.3)" }}>
          <p className="text-sm">Sign in to use chat.</p>
          <button type="button" onClick={() => setOpen(false)} className="mt-3 text-sm underline">Close</button>
        </div>
      ) : !myUserId ? (
        <div className={`w-[360px] h-[200px] rounded-lg border-2 flex items-center justify-center ${barBg} ${barBorder} ${barText}`} style={{ boxShadow: "0 10px 40px rgba(0,0,0,0.3)" }}>
