...
export default function RoomClient({
  ...
  const startScreenSharing = async () => {
    try {
      ...
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        ...
        // User canceled the screen-share picker — not an error, nothing to log.
      } else {
  ...
  const handleLeave = async () => {
    ...
    // Navigate back
    router.push(`/community/teams/${teamId}`);
  };
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${bgColor} ${textColor}`}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 w-12 border-b-2 border-zinc-600 mx-auto mb-4"></div>
          <p>Joining room...</p>
        </div>
      </div>
    );
      ...
      <div className={`min-h-screen flex items-center justify-center ${bgColor} ${textColor}`}>
        <div className={`${cardBg} ${borderColor} border rounded-lg p-6 max-w-md`}>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => router.push(`/community/teams/${teamId}`)}>
              Go Back
            </Button>
          </div>
        </div>
            ...
            <div className="absolute bottom-2 left-2 flex items-center gap-2">
              ...
              {isSharingScreen && (
                <Badge variant="info" size="sm" className={
                  colorMode === "light" ? "bg-zinc-900 text-white" : "bg-zinc-100 text-zinc-900"
                }>
                  Sharing
                </Badge>
              )}
